...
const ayarlar = require('./cliented.json');
const fs = require('fs');
...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: (ayarlar.altbaslık) }, status: "idle" });
...
client.on("message", async message => {
  ...
  let embed = new MessageEmbed().setColor("Random").setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, })).setFooter("Murphy was here?").setTimestamp();
  
  if (command === "geval" && message.author.id === ayarlar.botOwner) {
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "Zeox Guard Tarafından Banlandı." }).catch();
};
